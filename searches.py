    ...
    while True:
        ...
        try:
            ...
            if year < 0 or len(year_input) != 4:
                print(
                    "Год должен быть четырёхзначным положительным "
                    "числом. Попробуйте снова."
                )
                continue
...
def _add_film_to_favorites(choice, films, offset=0):
    ...
    Параметры:
        ...
        offset: Смещение для нумерации
                (0 для show_actor_films, >0 для поисковых функций)
    ...
    try:
        converted_choice = convert_layout_to_english(choice)  # input_utils.py
        film_num = int(converted_choice[1:])        # Проверка диапазона в зависимости от offset
        if offset == 0:
            # Для handle_actor_films: нумерация от 1 до len(films)
            if 1 <= film_num <= len(films):
                film = films[film_num - 1]
            else:
                print(f"Неверный номер фильма (доступно: 1-{len(films)})")
                return False
        else:
            ...
            if offset + 1 <= film_num <= offset + len(films):
                ...
                available = f"{offset + 1}-{offset + len(films)}"
                print(f"Неверный номер фильма (доступно: {available})")
                return False
        
        add_to_favorites(  # favorites.py
            film.get('film_id'),
            film.get('title'),
        ...
        print("Неверный формат. Используйте: f<номер> (например, f3)")
        return False


def _paginate_keyword_results(
        total, keyword, genre_id, year_min, year_max, age_rating):
    """Постраничный вывод результатов поиска по ключевому слову."""
    offset = 0
    while True:
        films = search_by_keyword(  # mysql_connector.py
            keyword=keyword,
            offset=offset,
            limit=LIMIT,
            genre_id=genre_id,
        ...
        offset = _show_films_page(films, offset, total)
        if offset is None:
            break


def _paginate_genre_results(total, genre_id, year_min, year_max, age_rating):
    """Постраничный вывод результатов поиска по жанру и годам."""
    offset = 0
    while True:
        films = search_by_genre_and_year(  # mysql_connector.py
            genre_id=genre_id,
            year_min=year_min,
            year_max=year_max,
            offset=offset,
...
def _show_films_page(films, offset, total):
    """Показывает одну страницу фильмов и обрабатывает действия пользователя.
    
    Возвращает:
        int: Новое значение offset для следующей страницы
        None: Если нужно выйти из пагинации
    """
    ...
    if total is not None:
        ...
        print_movies_table(  # formatter.py
            films,
            offset=offset,
            total=total,
            show_header=False)
        ...
        print_movies_table(films, offset=offset, total=total)  # formatter.py
        print(SEPARATOR)
    
    # После показа страницы — позволяем выбрать фильмы
    user_pressed_enter = False
    exit_requested = False
    while True:
        prompt = (
            f"{start}–{end} из {total}. Введите номер фильма для "
            "просмотра актеров, f<номер> для добавления в избранное, "
            "Enter - продолжить, q - выход: "
        )
        choice = process_input(prompt)  # input_utils.py
        ...
        if choice.lower().startswith('f'):
            _add_film_to_favorites(choice.lower(), films, offset)  # searches.py
            continue
                    ...
                    while True:
                        ...
                        try:
                            ...
                            if 1 <= actor_idx <= len(actors):
                                ...
                                # Повторный вывод текущей страницы
                                print(f"\n{SEPARATOR_MINUS}\n")
                                print_movies_table(  # formatter.py
                                    films,
                                    offset=offset,
...
def show_actor_films(actor_id, actor_name):
    ...
    try:
        total = get_films_by_actor_count(actor_id)  # mysql_connector.py
    ...
    offset = 0
    while True:
        films = get_films_by_actor(actor_id, offset=offset, limit=LIMIT)  # mysql_connector.py
        ...
        choice = process_input(prompt).lower()  # input_utils.py
        if choice == 'q':
            break
        
        # Обработка команды добавления в избранное
        if choice.startswith('f'):
            _add_film_to_favorites(choice, films, offset=offset)  # searches.py
            continue
...
def search_by_keyword_interactive():
    ...
    # логируем)
    keyword = process_input(  # input_utils.py
        " Введите ключевое слово (Enter для поиска всех фильмов, "
        "q для отмены): "
    )
    if keyword.lower() == 'q':
    ...
    # Опциональные дополнительные фильтры
    genre_id = None
    year_min = year_max = None
    age_rating = None
    
    use_filters = process_yes_no_input(  # input_utils.py
        "\n Фильтровать по дополнительным параметрам? "
        "(жанр, год выпуска, возрастная категория) (y/n): "
    )
    
    if use_filters:
        # Выбор жанра (сразу показываем список)
        genres = get_genres()  # mysql_connector.py
        if not genres:
            print("\n  Список жанров пуст, фильтр по жанру пропущен.\n")
        else:
            print_genres(genres)  # formatter.py
            while True:
                try:
                    idx_input = input(
                        " Выберите номер жанра (или Enter для пропуска): "
                    ).strip()
                    if not idx_input:
                        break
                    idx = int(idx_input)
        ...
        # Опциональные года
        try:
            min_year, max_year = get_year_bounds()  # mysql_connector.py
            print(f"\n Доступные годы: {min_year} — {max_year}")
            
            # Ввод нижнего года с валидацией (по умолчанию min_year)
            year_input = _get_year_input(f" Нижний год (Enter для {min_year}): ", min_year, max_year, allow_empty=True)  # searches.py
            y1 = year_input if year_input is not None else int(min_year)
            
            # Ввод верхнего года с валидацией (по умолчанию max_year)
            year_input = _get_year_input(f" Верхний год (Enter для {max_year}): ", min_year, max_year, allow_empty=True)  # searches.py
        ...
        try:
            ratings = get_age_ratings()  # mysql_connector.py  # mysql_connector.py
            if ratings:
    ...
    try:
        total = get_keyword_count(  # mysql_connector.py
            keyword,
            genre_id=genre_id,
            year_min=year_min,
            year_max=year_max,
    ...
    log_search("keyword", params, int(total) if total is not None else 0)  # log_stats.py

    # Постраничный вывод результатов
    _paginate_keyword_results(  # searches.py
        total, keyword, genre_id, year_min, year_max, age_rating
    )
...
def search_by_genre_interactive():
    ...
    try:
        ...
        y1 = _get_year_input(f" Нижний год (Enter для пропуска): ", min_year, max_year, allow_empty=True)  # searches.py
        
        # Ввод верхнего года с валидацией (можно пропустить)
        y2 = _get_year_input(f" Верхний год (Enter для пропуска): ", min_year, max_year, allow_empty=True)  # searches.py
    ...
    age_rating = None
    try:
        ratings = get_age_ratings()  # mysql_connector.py
        if ratings:
            print("Доступные возрастные категории:")
            for i, r in enumerate(ratings, 1):
                desc = AGE_RATING_DESCRIPTIONS.get(r, "(описание отсутствует)")
    ...
    try:
        total = get_genre_year_count(  # mysql_connector.py
    ...
    log_search("genre_year", params, int(total) if total is not None else 0)  # log_stats.py

    # Постраничный вывод результатов
    _paginate_genre_results(  # searches.py
        total, genre_id, y1, y2, age_rating
    )
